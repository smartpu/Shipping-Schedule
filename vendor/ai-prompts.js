...
【周别数据合并计算规则】
**重要：跨年周别合并计算规则**
- 当遇到跨年周别时（如 2025/53 和 2026/01），需要检查这两个周别的时间区间是否加起来正好是7天
- 如果 2025/53 周的时间区间小于7天，且与 2026/01 周加起来正好是7天，则这两个周别应该合并计算为一周
- **在分析运力、派船数、供需关系时，必须将 2025/53 和 2026/01 的数据合并计算，不要认为这两周的运力都特别少**
- 例如：如果 2025/53 周显示运力为 16,393 TEU、派船 7 次，2026/01 周显示运力为 13,811 TEU、派船 6 次，合并后应该是一周运力 30,204 TEU、派船 13 次
- 在判断供需关系、运力趋势时，必须基于合并后的完整一周数据进行分析，而不是分别看两个不完整的周别
...
1. **正常班期识别**
   ...
   - 判断班期频率（周班、双周班、月班等）
   - 识别固定的班期规律
   - **特别注意**：如果遇到跨年周别（如 2025/53+2026/01），需要将这两个周别合并计算为一周来分析班期规律
...
**特别注意**：在进行供需矩阵判断时，如果遇到跨年周别（如 2025/53 和 2026/01），必须将这两个周别的运力和派船数合并计算后再进行判断。不要因为看到 2025/53 或 2026/01 单独显示的数据较少就误判为运力短缺，应该基于合并后的完整一周数据来判断供需关系。
...
- **在计算运力覆盖比例时，如果遇到跨年周别（如 2025/53 和 2026/01），必须将两个周别的运力合并计算，然后与订舱情况对比，而不是分别计算两个不完整周别的覆盖比例。**
...
| 区域 | 航线/船公司 | 正常班期模式 | 班期异常统计 | 船型变化 | 当周运力(TEU) | 未来四周平均运力(TEU) | 运力趋势 | 建议 |
|------|------------|------------|------------|---------|--------------|-------------------|---------|------|
